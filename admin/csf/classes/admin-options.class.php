...
if (!class_exists('CSF_Options')) {
  class CSF_Options extends CSF_Abstract
  {
    ...
    public $args         = array(
      ...
      // footer
      'footer_text'             => 'Thank you for using our product',
      'footer_after'            => '',
      'footer_credit'           => '',
    ...
    public function ajax_save()
    {
      ...
      if (!$result) {
        wp_send_json_error(array('error' => esc_html__('Error while saving the changes.', 'chat-viber')));
      } else {
    ...
    public function set_options($ajax = false)
    {
      ...
      $response  = ($ajax && !empty(wp_kses_post($_POST['data']))) ? json_decode(wp_unslash(trim(wp_kses_post($_POST['data']))), true) : $_POST;
      ...
      if (wp_verify_nonce($nonce, 'csf_options_nonce')) {
        ...
        if (!$ajax && !empty($response['csf_import_data'])) {
          ...
          $importing    = true;
          $this->notice = esc_html__('Settings successfully imported.', 'chat-viber');
        }
        ...
        if (!empty($transient['reset'])) {
          ...
          $this->notice = esc_html__('Default settings restored.', 'chat-viber');
        ...
        if (empty($this->notice)) {
          $this->notice = esc_html__('Settings saved.', 'chat-viber');
        }
    ...
    public function add_options_html()
    {
      ...
      echo ($this->args['show_form_warning']) ? '<div class="csf-form-result csf-form-warning">' . esc_html__('You have unsaved changes, save your changes!', 'chat-viber') . '</div>' : '';

      echo ($has_nav && $this->args['show_all_options']) ? '<div class="csf-expand-all" title="' . esc_html__('show all settings', 'chat-viber') . '"><i class="fas fa-outdent"></i></div>' : '';

      echo ($this->args['show_search']) ? '<div class="csf-search"><input type="text" name="csf-search" placeholder="' . esc_html__('Search...', 'chat-viber') . '" autocomplete="off" /></div>' : '';

      echo '<div class="csf-buttons">';
      echo '<input type="submit" name="' . esc_attr($this->unique) . '[_nonce][save]" class="button button-primary csf-top-save csf-save' . esc_attr($ajax_class) . '" value="' . esc_html__('Save', 'chat-viber') . '" data-save="' . esc_html__('Saving...', 'chat-viber') . '">';
      echo ($this->args['show_reset_section']) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="' . esc_html__('Reset Section', 'chat-viber') . '" data-confirm="' . esc_html__('Are you sure to reset this section options?', 'chat-viber') . '">' : '';
      echo ($this->args['show_reset_all']) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="' . (($this->args['show_reset_section']) ? esc_html__('Reset All', 'chat-viber') : esc_html__('Reset', 'chat-viber')) . '" data-confirm="' . esc_html__('Are you sure you want to reset all settings to default values?', 'chat-viber') . '">' : '';
      echo '</div>';
      ...
      if ($has_nav) {
        ...
        foreach ($this->pre_tabs as $tab) {
          ...
          if (!empty($tab['subs'])) {
            ...
            echo '<a href="#tab=' . esc_attr($tab_id) . '" data-tab-id="' . esc_attr($tab_id) . '" class="csf-arrow">' . esc_attr($tab_icon . $tab['title']) . esc_attr($tab_error) . '</a>';
            ...
            foreach ($tab['subs'] as $sub) {
              ...
              echo '<li><a href="#tab=' . esc_attr($sub_id) . '" data-tab-id="' . esc_attr($sub_id) . '">' . esc_html($sub_icon) . esc_html($sub['title']) . esc_html($sub_error) . '</a></li>';
            }
      ...
      foreach ($this->pre_sections as $section) {
        ...
        if (!empty($section['fields'])) {
          ...
          echo '<div class="csf-no-option">' . esc_html__('No data available.', 'chat-viber') . '</div>';
        }
      ...
      if (!empty($this->args['show_footer'])) {
        ...
        echo '<div class="csf-buttons">';
        echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save' . esc_attr($ajax_class) . '" value="' . esc_html__('Save', 'chat-viber') . '" data-save="' . esc_html__('Saving...', 'chat-viber') . '">';
        echo ($this->args['show_reset_section']) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="' . esc_html__('Reset Section', 'chat-viber') . '" data-confirm="' . esc_html__('Are you sure to reset this section options?', 'chat-viber') . '">' : '';
        echo ($this->args['show_reset_all']) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="' . (($this->args['show_reset_section']) ? esc_html__('Reset All', 'chat-viber') : esc_html__('Reset', 'chat-viber')) . '" data-confirm="' . esc_html__('Are you sure you want to reset all settings to default values?', 'chat-viber') . '">' : '';
        echo '</div>';
